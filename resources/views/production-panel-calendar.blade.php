...
<div class="row">
  ...
  <a href="#" class="btn btn-outline-dark btn-sm mb-1"><i class="fas fa-book"></i> <i class="fas fa-mug-hot"></i>&nbsp;&nbsp;Archiwum</a>
          ...
          <div class="card-footer rounded-3 border-start border-end border-bottom mt-3">
            ...
            @foreach ($days as $day)
            @if ($day['status'] != 0)
            <div class="row align-items-center bg-light border border-bottom border-light border-2 my-1">
            @else
            <div class="row align-items-center border-bottom border-light border-2 my-1">
            @endif
              <div class="col-md-3 mx-auto">
                ...
                <div class="btn-group text-right" role="group" aria-label="Third group">
                  ...
                  <input class="form-check-input mt-2" type="checkbox" id="{{$check_number = $check_number+1}}" name="check{{$check_number}}" value="{{$day['date']}}" @if ($day['status'] != 0) @else disabled @endif>
                  <label class="form-check-label" for="exampleRadios1">
                    
                  </label>
                  &nbsp;&nbsp;
                  <h4 class="blueiconcolor">{{$day['day']}}</h4>
                  
                  &nbsp;<small class="mt-2 grey600color">{{$day['name']}}</small></i>
                </div>
              </div>
              
              <div class="col-md-2 bg-transparent border-1 routed text-center mx-auto">
                @if ($day['status'] != 0)
                <a href="{{ route('production.get', ['action' => 'load', 'date' => 'data', 'temp_prod_id' => 0])}}" type="submit" name="action" value="load" class="btn btn-light btn-sm border border-1 routed routed-3"><i class="far fa-folder-open"></i><small>  </small></a>&nbsp;&nbsp;
              ...
              <div class="col-md-2 grey600color">
                @if ($day['status'] != 0)
                {{-- <button class="btn btn-sm" disabled><small>{{(\App\Models\ElementProduction::where('date_production', $day['date'])->first())->created_at->toDateTimeString()}}</small></button> --}}
                @endif
              </div>
